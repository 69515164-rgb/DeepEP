...
和 3.6 的切分点：有 CCL 时，这 5 轮就是 Mesh 上可挂钩的「拍」——第 i 轮收齐后可以改第 i+1 轮的 `tile→VT`。对称内存去掉这 5 轮后，同一次 Mesh 内仍有切分点，见 3.8，不是只剩「等下一次集体」。

### 3.8 对称内存窗口多大、会不会把算子内切分做没

**预分配多大**

| 平台 | 预留方式 | 默认大小 | 实际窗口 |
| --- | --- | --- | --- |
| Atlas A3 HCCS | `hcclSymWinMaxMemSizePerRank` 预留虚拟堆 | **每 rank 16GB**（可配 1GB～物理上限） | 通信域虚拟总量 `rankSize × 16GB`；真正能用的是 `HcclCommSymWinRegister` 映射的那块物理内存，且 `size ≤ 16GB` |
| 昇腾 950 URMA | **不走这个预留参数** | 无 16GB 默认堆 | 用已经申请好的 Device 内存注册窗口，窗口 = 这块物理内存 |

950PR HBM 128GB、950DT 144GB。窗口上限是「这块 tensor 占多少 HBM」，不是 CCL 的 200MB，也不是必须先掏出 16GB。

**够不够盖住典型消息**

对称窗口要盖的是**这一次算子的 send/recv buffer**，不是整网梯度、也不是所有通信域的峰值之和。

| 典型集体 | 单 rank 消息量级 | 16GB / 950 用户 HBM |
| --- | --- | --- |
| DDP / 梯度 bucket | 25～256MB | 够 |
| TP AllGather 一层激活 | 几十 MB～约 1GB | 够 |
| FSDP 一层 AG / RS | 约 0.2～2GB（按层融合） | 够 |
| 超节点内 EP AlltoAll | decode 很小；prefill 可达数百 MB～数 GB | 单次通常够 |
| 把 70B bf16 整模一次 AllReduce | ~140GB | **不够也不该一次塞**，业务侧本来就会切 bucket / 分层 |

结论：单次典型集体（百 MB～数 GB）窗口盖得住。不够的情况是「很多通信域各预留 16GB」或「想把整模参数+梯度+激活都放进一个对称堆」。A3 上把 `hcclSymWinMaxMemSizePerRank` 从 16 调到刚好大于本域最大 send+recv 即可。950 只注册实际 tensor，不要按 16GB 去理解。

**「只有一次下发」会不会让算子内动态切分没空间**

不会。对称内存消掉的是 **CCL 200MB 循环**，不是 Mesh 内部的并发度和硬件切片。

「一次下发」只表示：**逻辑上不再 for-loop 五轮 Mesh**。同一次 Mesh 里仍然有：

```text
1. 对端维     N-1 条并发链路（Mesh 从流）
2. VT/Jetty维 同一对端多条 Transport Channel
3. WQE 维     单次 put/DMA 上限（AIV/UDMA 路径常见单次 ≤256MB）
              1GB 对一个对端也会切成多条 WQE
4. 算法维     若实际走的是 Mesh 包一层 Ring/NHR，拍还在
```

动态切分挂钩的是 **2 和 3**（以及 Ring/HD 的拍），从来不是 CCL 循环本身。PReCCL 要的是「同一跳上慢 VT 少吃字节」，Mesh 一轮里 K 条 Jetty 同时打同一对端，正好是这个挂钩。没有 200MB 循环，切分空间反而更干净：不用把「scratch 轮次」和「路径极化」混在一起。

```text
有 CCL：     [轮1 Mesh] [改表] [轮2 Mesh] ...     ← 轮次屏障额外存在
对称内存：   一轮 Mesh {
               VT0: WQE, WQE, ...     ← 可按 VT 份额切
               VT1: WQE, WQE, ...
               ...
             }
             同一对端多 WQE 之间仍可收齐再改下一截份额
             整次 Wait 之后仍可改下一次集体
```

唯一变窄的是：不能再靠「第 3 轮 CCL 才改第 4 轮」这种粗粒度屏障。要切分就要在 **AI CPU 的 Mesh 从流循环 / 按 Tile 下 WQE 的循环** 里下手，或退回集体边界。CCU_SCHED 若把 1GB 收成一条 Mission 条带多 Jetty，必须先拆成「一 Mission 一 Jetty」，否则对称内存下一轮 Mesh 也切不动。
