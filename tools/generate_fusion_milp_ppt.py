...
def title_slide(prs):
    ...
    add_text(s, "把集合通信 MILP 求解器扩展到「边算边传」：\n计算编译器给定切块和计算顺序，求解器只编排通信。",
...
def build():
    ...
    s = base_slide(prs, "今天要拍的板：求解器不搜计算，只排通信", "结论先行")
    card(s, "建议采用", "计算编译器给两样：\n  tiling（空间怎么切）\n  计算流图（这些块何时算完）\n通信 MILP 只决定：\n  算完的数据何时发给谁",
         0.6, 1.35, 6.05, 3.45, GREEN, 16, 15)
    card(s, "不建议", "不要让通信求解器去选矩阵乘的切块大小\n不要让它重排计算顺序\n那是另一类编译器，塞进同一个方程会又慢又不准",
         6.9, 1.35, 5.85, 3.45, RED, 16, 15)
    note(s, "MILP = 混合整数线性规划。工业界用它自动生成集合通信的「谁在何时发给谁」。代表工作：TACCL、TE-CCL、SyCCL。它们从来只搜通信，不搜矩阵乘怎么切。",
         0.6, 5.05, 12.15, 1.45)
    ...
    note(s, "ready = 写完 + 2 μs（写到通信可见的固定延迟）。第一包 12 μs 才能进 AllReduce，不是 0 时刻。若 wave=2（C00 与 C01 同时算），两块都是 12 μs 就有货——切块相同，出炉时刻不同，所以必须给流图。",
         0.6, 4.2, 12.15, 2.2)
    ...
    bullets(s, [
        "求解器可以决定：某块数据 ready 之后，走哪条链路、哪个时刻发出",
        "求解器不可以决定：切块长宽、计算顺序、激活做在 AllReduce 前还是后",
        "AllReduce 的邻居关系（Ring / HD）建议一并给定，求解器不换算法",
    ], 0.7, 4.2, 12, 1.55, 15)
    note(s, "和上一页合起来：我们扩展的是「数据什么时候存在」，不是扩展求解器去当计算编译器。",
         0.6, 5.85, 12.15, 1.05)
    ...
    card(s, "交出来", "通信时刻表：哪一包、何时、走哪条路\n保证：没有任何一包早于它的 ready\n\n不交：新的切块方案、新的计算顺序",
         6.9, 1.28, 5.85, 3.35, GREEN, 16, 14)
    note(s, "目标函数从「通信越早结束越好」改成「整个融合算子越早结束越好」。因为计算收工时间已经由流图钉死，求解器真正能抠的是：通信怎样咬着计算的尾巴往前叠。",
         0.6, 4.9, 12.15, 1.55)
    ...
    add_text(s, "立项口径：扩展现有集合通信求解器，支持「带就绪时间的 AllReduce」；计算切块仍由计算编译器给定。",
             0.7, 1.25, 12, 0.7, 17, WHITE, True)
    for i, (n, t, b, c) in enumerate([
        ("01", "冻结接口", "tiling + 计算流图\n两份输入写进合同", BLUE),
        ("02", "最小内核改动", "原求解器加一条\n「未就绪不准发」", CYAN),
        ("03", "先冻通信算法", "Ring/HD 给定\n只排重叠时刻", GREEN),
        ("04", "用总时间验收", "对比两阶段基线\n数值必须一致", ORANGE),
    ]):
        x = 0.6 + i * 3.15
    ...
    note(s, "详细输入合同见 docs/fusion_milp_input.md。本页是给决策用的边界：谁负责切块，谁负责发数据，互不越权。",
         0.6, 5.05, 12.15, 1.4)
    ...
    rows = [
        ...
        ("MILP 综合器", "自动排出集合通信时刻表的整数规划器"),
        ("融合算子", "同一段内核里边算矩阵乘边做 AllReduce"),
        ("ready", "某一包数据对通信可见的最早时刻（由流图推出）"),
        ("Ring / HD", "AllReduce 的两种经典传法；本方案建议给定、不搜"),
    ]
    for i, (t, b) in enumerate(rows):
        y = 1.18 + i * 0.60
        add_text(s, t, 0.7, y, 3.3, 0.48, 13, CYAN, True)
        add_text(s, b, 4.1, y, 8.5, 0.48, 14, LIGHT)
